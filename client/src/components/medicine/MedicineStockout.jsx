...
class MedicineStockin extends React.Component {
    ...
    remove(index) {
        ...
        axios.post('/api/stockout/remove', {
            _id: data[index]._id,
        })
    ...
    onPageChange(pageNumber) {
        ...
        .then(function (response) {
            var stockouts = response.data.msg.data;
            var data = [];

            for (var i = 0; (i < stockouts.length) && (i < 10); i++) {
                data.push({
                    key: i,
                    editable: false,
                    _id: stockouts[i]._id,
                    name: stockouts[i].name,
                    standard: stockouts[i].standard,
                    dose: stockouts[i].dose,
                    unit: stockouts[i].unit,
                    price: stockouts[i].price,
                    freqdaily: stockouts[i].freqdaily,
                    days: stockouts[i].days,
                    total: stockouts[i].total,
                    money: stockouts[i].money,
                    created: moment(stockouts[i].created).format('YYYY-MM-DD HH:mm:ss'),
                });
            }
    ...
    render() {
        ...
        return (
            <div className="gutter-example">
                <BreadcrumbCustom first="药品" second="开药记录" />
                <Row gutter={16}>
